...
export const AC_SERVICE_WALKTHROUGH = [
  { title: '1. Lock the car and refrigerant configuration', target: 'Read the under-hood labels, installed fitting adapters, receiver label and service history before selecting a procedure.', detail: 'The baseline is a 1990 UCF10L-AEPGKA with R-12 context, but VIN/build month and installed hardware control the exact parts. Unknown, mixed or contaminated refrigerant is a hard stop: identify it professionally before connecting recovery equipment.', primaryId: 'AC_RECEIVER_DRIER', camera: 'RECEIVER_DRIER_CLOSE', isolation: 'AC_ENGINE_BAY', landmarks: 'Passenger headlight → radiator support → receiver/drier top block.', boundaries: 'Do not treat fittings or labels alone as proof of refrigerant or oil type.', source: 'RM144U service information; job_ac_service configuration profiles' },
  { title: '2. Choose the correct branch: original R-12 or verified Lexus R-134a retrofit', target: 'Use the verified configuration to select materials and charging data; do not mix the two oil specifications.', detail: 'Factory R-12 is 1,050 g / 2.3 lb with mineral oil context. The Lexus AC001-98 retrofit branch specifies R-134a, 1,000 g, ND-OIL 8 PAG, receiver 88471-16050, both service adapters, listed O-rings and retrofit labels. AC001-98 says flushing original mineral oil is not required for that approved retrofit.', primaryId: 'AC_HIGH_SERVICE_PORT', camera: 'RECEIVER_DRIER_CLOSE', isolation: 'AC_HIGH', landmarks: 'Receiver-side liquid pipe and service fitting at the passenger-front support.', boundaries: 'After retrofit, a cloudy sight glass is expected; charge by specified mass, never sight-glass clarity.', source: 'Lexus AC001-98; job_ac_service configuration profiles' },
  { title: '3. Recover, identify and verify zero pressure before any joint is considered', target: 'This is the safety gate that unlocks planning boundaries in the model.', detail: 'Recovery must occur before a repair expected to release refrigerant. Never vent R-12, R-134a or substitutes. Cap every opening once a qualified technician has recovered the verified refrigerant and independently confirmed zero retained pressure.', primaryId: 'AC_LOW_SERVICE_PORT', camera: 'PASSENGER_FIREWALL', isolation: 'AC_COMPLETE', landmarks: 'Low-side suction assembly in the passenger-side rear engine bay; high-side liquid/service fitting at the passenger-front receiver area.', boundaries: 'The service ports are equipment connections, not flush-injection points or proof that the system is empty.', source: 'EPA MVAC boundary; job_ac_service safety gate' },
  { title: '4. Expose and replace the receiver/drier — do not flush through it', target: 'Work from the passenger-front corner, behind the right/passenger headlamp, after recovery.', detail: 'The factory removal path calls for right-headlamp access, then both liquid tubes are disconnected and capped. The receiver/drier is a replace-not-flush component; the manual-derived torque data lists 5.4 N·m for the receiver liquid-tube joints. Keep installed part and retrofit history visible in the decision record.', primaryId: 'AC_RECEIVER_DRIER', camera: 'RECEIVER_DRIER_CLOSE', isolation: 'AC_HIGH', landmarks: 'Passenger headlight → receiver clamp → upper liquid-pipe block → radiator support.', boundaries: 'Receiver inlet and outlet are the external line boundaries; the canister itself stays out of a solvent path.', routes: ['AC_LIQUID_LINE_CONDENSER_DRIER', 'AC_LIQUID_LINE_DRIER_FIREWALL'], requiresPlanningGate: true, source: 'RM144U receiver removal/installation; EPC front piping' },
  { title: '5. Isolate the compressor at both manifold ports — do not generic-flush it', target: 'Locate the low front driver-side accessory-drive compressor and identify its two different ports.', detail: 'The small discharge connection begins the high side; the larger suction connection returns low-pressure vapor. Factory engine work can remove the compressor without disconnecting hoses, which is a useful orientation clue, not permission to open a charged circuit. A replacement compressor’s oil accounting must follow the verified configuration and maker instructions.', primaryId: 'AC_COMPRESSOR', camera: 'AC_COMPRESSOR_CLOSE', isolation: 'AC_ENGINE_BAY', landmarks: 'Driver-side lower front of 1UZ-FE → bracket → clutch/pulley → large suction and smaller discharge connections.', boundaries: 'Use the suction and discharge joints only as isolation boundaries; never push generic flush solvent through the compressor.', routes: ['AC_DISCHARGE_LINE', 'AC_SUCTION_LINE'], requiresPlanningGate: true, source: 'RM144U engine/compressor removal; EPC compressor plate' },
  { title: '6. Find the condenser and its two front-end joints', target: 'View the condenser in the front heat-exchanger stack before tracing either line.', detail: 'The condenser is behind the bumper/grille and ahead of the radiator, with the paired electric condenser fans in the front stack. Factory removal references bumper, two fans, center brace/horns and recovered/capped liquid and discharge tubes. The documented condenser discharge and liquid tube torque is 10 N·m; actual replacement-core design decides whether it is flushable.', primaryId: 'AC_CONDENSER', camera: 'CONDENSER_STACK', isolation: 'AC_HIGH', landmarks: 'Grille/bumper → fan pair → condenser → radiator; do not confuse the stack with the receiver behind the passenger headlamp.', boundaries: 'A parallel-flow/multiflow replacement core, severe metal contamination or leak-stop contamination takes the replacement path, not a solvent-flush assumption.', routes: ['AC_DISCHARGE_LINE', 'AC_LIQUID_LINE_CONDENSER_DRIER'], requiresPlanningGate: true, source: 'RM144U condenser removal/installation; EPC condenser/fans' },
  { title: '7. Trace only bare external line sections, one end-to-end section at a time', target: 'Use route highlighting to distinguish the discharge, condenser-to-receiver, receiver-to-firewall and suction groups.', detail: 'The EPC establishes hose/tube group identity, but not every installed bend, clamp clocking or internal restriction. Keep each removed section labeled by both endpoints and nearby landmarks. Bare, verified-unrestricted lines may be evaluated under the selected vendor procedure; sections containing a muffler, filter or unknown internal restriction are replacement candidates.', primaryId: 'AC_LIQUID_LINE_DRIER_FIREWALL', camera: 'PASSENGER_FIREWALL', isolation: 'AC_LINES', landmarks: 'Compressor discharge → condenser → passenger-front receiver → passenger firewall; EPR/suction route returns to the compressor.', boundaries: 'The animated arrows show normal operating flow, never solvent direction or an approved flush path.', routes: ['AC_DISCHARGE_LINE', 'AC_LIQUID_LINE_CONDENSER_DRIER', 'AC_LIQUID_LINE_DRIER_FIREWALL', 'AC_SUCTION_LINE'], requiresPlanningGate: true, source: 'UCF10 EPC piping; job_ac_service graph' },
  { title: '8. Stop at the expansion valve and EPR; expose the cabin-side context', target: 'Follow the passenger-side firewall area into the HVAC case without pretending hidden routing is measured.', detail: 'The expansion valve is in the cooling-unit/evaporator case and the EPR sits at the evaporator outlet/firewall area. The manual separates the expansion valve from the evaporator during case work and explicitly disconnects the EPR equalizer tube. Treat both as controls to isolate or replace, not flush-through fittings.', primaryId: 'AC_EXPANSION_VALVE', camera: 'HVAC_CASE_CUTAWAY', isolation: 'AC_CABIN', landmarks: 'Passenger firewall pass-through → expansion valve/HVAC case → evaporator → EPR → suction return.', boundaries: 'The equalizer tube is a control branch, not a main-flow hose; its exact far endpoint remains a physical-trace item.', routes: ['AC_EVAPORATOR_FEED_INTERNAL', 'AC_EVAPORATOR_RETURN_INTERNAL', 'AC_EQUALIZER_TUBE', 'AC_SUCTION_LINE'], requiresPlanningGate: true, source: 'RM144U evaporator/case disassembly; EPC cooler unit' },
  { title: '9. Reassemble with configuration-matched parts, oil and sealing data', target: 'Use only the branch selected in Step 2 and the installed-car inspection record.', detail: 'For factory R-12 component replacement, oil additions are component-specific: receiver 20 cc; condenser or evaporator 40–50 cc; pipe over 3 ft 5 cc. Do not combine that schedule with the retrofit PAG quantity. Factory joint values represented here are 32 N·m for condenser suction tube, 10 N·m for condenser discharge/liquid, evaporator liquid/suction and EPR equalizer, and 5.4 N·m for receiver liquid tubes.', primaryId: 'AC_EPR', camera: 'PASSENGER_FIREWALL', isolation: 'AC_COMPLETE', landmarks: 'Verify each capped joint against its highlighted component and local landmark before reassembly.', boundaries: 'Torque applies to the named factory joint, not every visually similar fitting; confirm the exact procedure and installed part.', source: 'RM144U specifications; job_ac_service factory_service_specs' },
  { title: '10. Evacuate, leak-test and charge by verified specification', target: 'Close the loop only after every cap, fitting, O-ring and replaced component is documented.', detail: 'Evacuation, leak testing, charging and final performance verification belong to the qualified procedure and the verified refrigerant branch. Charge by specified mass, not by the R-134a retrofit sight glass. Preserve labels, record the refrigerant/oil used, and stop for an unexplained leak, restriction or contamination result.', primaryId: 'AC_HIGH_SERVICE_PORT', camera: 'FRONT_PASSENGER_CORNER', isolation: 'AC_COMPLETE', landmarks: 'High-side receiver/front support area and low-side suction return are the service-equipment reference points.', boundaries: 'Never open a high-side manifold valve with the engine running or charge liquid refrigerant into a running system.', source: 'Lexus AC001-98; MVAC safety boundary' }
];
